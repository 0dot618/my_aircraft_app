...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    public boolean isGameOver() {
        if(heroAircraft.notValid()) return true;
        else return false;
    }
...
//            }
    ...
    private void paintScoreAndLife() {
        ...
        canvas.drawText("SCORE:",60,80,spaint);
        canvas.drawText(String.valueOf(score),300,80,spaint);
        canvas.drawText("LIFE:",60,200,spaint);
        canvas.drawText(String.valueOf(heroAircraft.getHp()),250,200,spaint);
        if(OnlineActivity.bothOnline) {
            canvas.drawText("OPPONENT_SCORE:",60,320,spaint);
            canvas.drawText(String.valueOf(OnlineActivity.getOpponentScore()),650,320,spaint);
        }
    }
    ...
    public void run() {
        /*TODO*/
        mp = new MyMediaPlayer(context,musicOn);
        mp.startBgMP();
        mysp = new MySoundPool(context,musicOn);
        while(mIsDrawing){
            try {
                this.action();
            } catch (IOException e) {
        ...
        while(OnlineActivity.bothOnline) {
            draw();
        }
    }
}
